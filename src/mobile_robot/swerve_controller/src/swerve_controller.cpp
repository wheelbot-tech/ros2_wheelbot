...
namespace swerve_controller
{
  ...
  controller_interface::CallbackReturn SwerveController::on_init()
  {
    try
    {
      ...
      auto_declare<double>("y_axis_direction", y_axis_direction_);
      auto_declare<double>("yaw_axis_direction", yaw_axis_direction_);
      ...
      const rclcpp::Time &time, const rclcpp::Duration & period)
  {
    ...
    // Inversează sensul pentru axa Y si yaw - necesar la simularea in Isaac Sim **************************
    linear_y_velocity_comand *= y_axis_direction_;
    angular_velocity_comand *= yaw_axis_direction_;
    ...
    double turn_angle = atan2(wheel_params_.y_offset, wheel_params_.x_offset);

    // No movement is given
    if (std::abs(linear_x_velocity_comand) <= 0.1 && std::abs(linear_y_velocity_comand) <= 0.1 && 
        std::abs(angular_velocity_comand) <= 0.1)
    {
      // Make position current 
      ...
      // Set wheels to block the robot
      front_left_position = M_PI_2;  
      front_right_position = 0.0;
      rear_left_position = 0.0;
      rear_right_position = M_PI_2;
    } 
    // only positive angular velocity movement is given
    else if (std::abs(linear_x_velocity_comand) <= 0.1 && std::abs(linear_y_velocity_comand) <= 0.1 
             && angular_velocity_comand > 0.1)
    {
      front_left_position = M_PI - turn_angle;   
      front_right_position = turn_angle;        
      rear_left_position = -M_PI + turn_angle;  
      rear_right_position = -turn_angle;        
    }
    // only negative angular velocity movement is given
    else if (std::abs(linear_x_velocity_comand) <= 0.1 && std::abs(linear_y_velocity_comand) <= 0.1 
             && angular_velocity_comand < -0.1)
    {
      front_left_position = -turn_angle;        
      front_right_position = -M_PI + turn_angle; 
      rear_left_position = turn_angle;          
      rear_right_position = M_PI - turn_angle;  
    }
    else {
  ...
  controller_interface::CallbackReturn SwerveController::on_configure(const rclcpp_lifecycle::State &)
  {
    ...
    y_axis_direction_ = get_node()->get_parameter("y_axis_direction").as_double();
    yaw_axis_direction_ = get_node()->get_parameter("yaw_axis_direction").as_double();
