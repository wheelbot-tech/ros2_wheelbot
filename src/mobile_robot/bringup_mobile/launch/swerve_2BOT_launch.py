...
def generate_launch_description():
    ...
    robot_description_content = Command(
        [
            ...
            PathJoinSubstitution(
                [FindPackageShare("description"), "urdf/robots", "swerve_2BOT.urdf.xacro"]
            ),
        ]
    )
    ...
    robot_controllers = PathJoinSubstitution([FindPackageShare("bringup"), "config", "swerve_2BOT_controllers.yaml"])
    ...
    control_node_remapped = Node(
        ...
        remappings=[
            ...
            ("/swerve_drive_controller/tf_odometry", "/tf"),
        ],
    ...
    delay_drive_controller_spawner_after_swerve_controller_spawner = RegisterEventHandler(
        event_handler=OnProcessExit(
            ...
            on_exit=[FR_drive_controller_spawner,  RL_drive_controller_spawner],
        )
    )

    convertor_node = Node(
        package="bringup",
        executable="jointstate_to_twist_2_swerve",
        name="jointstate_to_twist_2_swerve",
        output="screen",
        parameters=[{
    ...
    jointstate_aggregator_node = Node(
        package='bringup',
        executable='jointstate_aggregator_2_swerve',
        name='jointstate_aggregator_2_swerve',
        output="screen",
    )
