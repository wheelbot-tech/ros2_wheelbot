...
public:
    ...
    fr_drive_topic(std::string(this->get_namespace()) + "/FR_drive_joint_states"),
    rl_drive_topic(std::string(this->get_namespace()) + "/RL_drive_joint_states"),
    amr_state_topic(std::string(this->get_namespace()) + "/amr_joint_states"),
    isaac_state_topic(std::string(this->get_namespace()) + "/isaac_joint_states")
   ...
   void publish_drive_joint_state(const std::string& topic, double pos_left, double pos_right, double vel_left, double vel_right)
    ...
    void joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
    {
        ...
        new_msg_amr.position = {
            msg->position[0],  // FR_steering_joint (virtual_front_right_steering_joint)
            (msg->position[6] + msg->position[7]) / 2.0,  // FR_drive_left_joint + FR_drive_right_joint (virtual_front_right_wheel_joint)
            msg->position[1],  // mimic RL_steering_joint (virtual_front_left_steering_joint)
            (msg->position[8] + msg->position[9]) / 2.0,  // FL_drive_left_joint + FL_drive_right_joint (virtual_front_left_wheel_joint)
            msg->position[0],  // mimic FR_steering_joint (virtual_rear_right_steering_joint)
            (msg->position[6] + msg->position[7]) / 2.0,  // RR_drive_left_joint + RR_drive_right_joint (virtual_rear_right_wheel_joint)
            msg->position[1],  // RL_steering_joint (virtual_rear_left_steering_joint)
        ...
        new_msg_amr.velocity = {
            msg->velocity[0],  // FR_steering_joint (virtual_front_right_steering_joint)
            (msg->velocity[6] + msg->velocity[7]) / 2.0,  // FR_drive_left_joint + FR_drive_right_joint (virtual_front_right_wheel_joint)
            msg->velocity[1],  // mimic RL_steering_joint (virtual_front_left_steering_joint)
            (msg->velocity[8] + msg->velocity[9]) / 2.0,  // FL_drive_left_joint + FL_drive_right_joint (virtual_front_left_wheel_joint)
            msg->velocity[0],  // mimic FR_steering_joint (virtual_rear_right_steering_joint)
            (msg->velocity[6] + msg->velocity[7]) / 2.0,  // RR_drive_left_joint + RR_drive_right_joint (virtual_rear_right_wheel_joint)
            msg->velocity[1],  // RL_steering_joint (virtual_rear_left_steering_joint)
